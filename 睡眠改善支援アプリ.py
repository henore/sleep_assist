...
class DatabaseManager:
    ...
    def execute_query(self, query, params=None):
        ...
        try:
            ...
            print(f"Debug: SQL Query: {query}, Params: {params}, Result: {result}")  # デバッグ出力
            return result
    ...
    def get_all_advice_history(self):
        query = "SELECT * FROM advice_history"
        result = self.execute_query(query)
        print("Debug: All advice history:", result)
        return result
...
class SleepRecordManager:
    ...
    def calculate_sleep_duration(self, sleep_time, wake_time):
        ...
        if len(sleep_time.split(':')) == 2:
            sleep_time += ":00"  # 秒がない場合は追加
        if len(wake_time.split(':')) == 2:
            wake_time += ":00"  # 秒がない場合は追加
        ...
        try:
            ...
            print(f"無効な時間形式: {e}")
            return "無効な時間フォーマットです。正しい形式で入力してください。"
...
class UIManager:
    def __init__(self, master):
        self.master = master
        self.master.title("睡眠改善支援アプリ")
        self.master.geometry("800x700")

    def create_main_window(self):
        self.profile_button = ttk.Button(self.master, text="プロフィール表示/編集")
        self.profile_button.pack(pady=10)
    
        self.cbt_info_button = ttk.Button(self.master, text="睡眠について悩んでおられる方へ")
        self.cbt_info_button.pack(anchor="nw", padx=10, pady=10)

        self.create_sleep_input_fields()

        self.history_button = ttk.Button(self.master, text="睡眠履歴とAIの助言を振り返る")
    ...
    def show_recent_history(self, recent_records, delete_callback, show_advice_callback):
        ...
        warning_text = "※ AIの助言は睡眠に関する一般的な参考情報であり、必ずしも正確とは限りません。判断に迷う場合や症状が辛い時は、必ず医師に相談してください。"
        ...
        for record in recent_records:
            self.create_recent_record_display(scrollable_frame, record, delete_callback)  # delete_callbackを渡す

        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")

    def create_recent_record_display(self, parent_frame, record, delete_callback):
        ...
        if record[11]:  # AI助言がある場合（advice_history_idが存在する場合）
            ...
            ttk.Button(info_frame, text="アドバイスを表示", 
        ...
        ttk.Button(button_frame, text="削除", 
                   command=lambda: delete_callback(record[0])).pack(side="top", pady=2)
    ...
    def refresh_calendar(self, all_dates):
        self.cal.calevent_remove('all')
        
        for date_tuple in all_dates:
            date_str = date_tuple[0]
            date_obj = datetime.strptime(date_str, "%Y-%m-%d").date()
            self.cal.calevent_create(date=date_obj, text="AI助言あり", tags="advice")
        
        self.cal.tag_config('advice', background='lightblue')
...
class AIAdviceManager:
    ...
    def _create_prompt(self, sleep_data, user_profile):
        ...
        return f"""
        あなたは共感力の高い睡眠の最新研究科学者です。ユーザーの睡眠データを分析し、個別化された助言を提供してください。
...
class SleepTherapyApp:
    def __init__(self, master):
        ...
        self.sleep_record_manager = SleepRecordManager(self.db_manager)
        ...
        # ユーザープロフィールの読み込み
        self.current_user = self.user_profile_manager.get_user_profile(1)  # 仮のユーザーID
        ...
        print("All advice history:")
        print(self.ai_advice_manager.get_advice_history())
    ...
    def bind_events(self):
        ...
        self.ui_manager.sleep_button.config(command=self.show_sleep_preparation)
    ...
    def show_cbt_info(self):
        ...
        if not cbt_content:
            ...
            cbt_content = cbt_content[0][0]  # 結果はリストのタプルで返されるため
    ...
    def show_ai_advice_for_record(self, date):
        advice = self.ai_advice_manager.get_advice_for_date(date)
        if advice:
    ...
    def save_sleep_preparation(self, preparation_text):
        # 睡眠準備情報の保存ロジックをここに実装
        sleep_date = self.ui_manager.sleep_date_entry.get()
        sleep_time = self.ui_manager.sleep_time_entry.get()
        self.sleep_time = f"{sleep_date} {sleep_time}"
        self.sleep_preparation_data = {"preparation_text": preparation_text}
    ...
    def record_wake(self):
        if not hasattr(self, 'sleep_time'):
            self.ui_manager.show_message("就寝時間が記録されていません。", "warning")
            return
    ...
    # データベースから全ての日付を取得
        all_dates = self.db_manager.execute_query("SELECT DISTINCT date FROM advice_history")
        
        # UIManagerのrefresh_calendarメソッドを呼び出す
        self.ui_manager.refresh_calendar(all_dates)
    ...
    def show_recent_history(self):
        recent_records = self.sleep_record_manager.get_recent_records()
